...
graph TD
    ...
    subgraph FileSystemGlobals
        direction LR
        SB(Superblock)
        INODES(Inodes Array)
        DB(Data Blocks Array)
        DIRS(Directories Array)
        CWD_INODE(Current Working Directory Inode)
        CWD_PATH(Current Working Directory Path)
    end

    subgraph CoreFSFunctions
        direction LR
        FS_INIT[initialize_fs]
    ...
    B -->|Command & Arguments| Commands;
    Commands -->|Output| E[User Output];

    B -->|Initial Setup| FS_INIT;
    FS_INIT --> SB;
    FS_INIT --> INODES;
    FS_INIT --> DIRS;
    FS_INIT --> CWD_INODE;
    ...
    CREATE_ROOT --> CREATE_INODE;
    CREATE_ROOT --> SB;
    CREATE_ROOT --> DIRS;
    CREATE_INODE --> SB;
    CREATE_INODE --> INODES;

    CMD_TOUCH -->|Path, Content| CREATE_FILE;
    CMD_MKDIR -->|Path| CREATE_DIR;
    CMD_LS -->|Path| TRAVERSE_PATH;
    CMD_LS --> DIRS;
    CMD_TREE -->|Path| TRAVERSE_PATH;
    CMD_TREE --> DIRS;
    CMD_TREE --> INODES;
    CMD_READ -->|Path| TRAVERSE_PATH;
    CMD_READ --> INODES;
    CMD_READ --> DB;
    CMD_DETAIL -->|Path| TRAVERSE_PATH;
    CMD_DETAIL --> INODES;
    CMD_RM -->|Path| TRAVERSE_PATH;
    CMD_RM --> RM_FILE;
    CMD_RMDIR -->|Path| TRAVERSE_PATH;
    CMD_RMDIR --> RM_DIR;
    CMD_CD -->|Path| TRAVERSE_PATH;
    CMD_CD --> CWD_INODE;
    CMD_CD --> CWD_PATH;
    CMD_PWD --> CWD_PATH;

    CREATE_FILE[create_file] --> TRAVERSE_PATH;
    CREATE_FILE --> CREATE_INODE;
    CREATE_FILE --> ALLOCATE_BLOCK;
    CREATE_FILE --> DIRS;
    CREATE_FILE --> INODES;
    CREATE_FILE --> DB;

    CREATE_DIR[create_dir] --> TRAVERSE_PATH;
    CREATE_DIR --> CREATE_INODE;
    CREATE_DIR --> DIRS;

    TRAVERSE_PATH --> SB;
    TRAVERSE_PATH --> CWD_INODE;
    TRAVERSE_PATH --> DIRS;
    TRAVERSE_PATH --> INODES;
    ...
    Commands -->|Access/Modify| SB;
    Commands -->|Access/Modify| INODES;
    Commands -->|Access/Modify| DB;
    Commands -->|Access/Modify| DIRS;
    Commands -->|Access/Modify| CWD_INODE;
    Commands -->|Access/Modify| CWD_PATH;

    classDef DataStore fill:#ace,stroke:#333,stroke-width:2px;
    class SB,INODES,DB,DIRS,CWD_INODE,CWD_PATH DataStore;